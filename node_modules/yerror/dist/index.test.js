...
describe('YError', () => {
    ...
    describe('.wrap()', () => {
        test('Should work with standard errors and a message', () => {
            ...
            if ('captureStackTrace' in Error) {
                expect(-1 !== (err.stack || '').indexOf('Error: This is an error!')).toBeTruthy();
                expect(-1 !== (err.stack || '').indexOf('YError: E_UNEXPECTED ([])')).toBeTruthy();
                expect(err.name).toEqual(err.toString());
            }
        });
        test('Should work with standard errors and an error code', () => {
            ...
            if ('captureStackTrace' in Error) {
                ...
                expect(-1 !== (err.stack || '').indexOf('YError: E_ERROR ([])')).toBeTruthy();
            }
            expect(err.name).toEqual(err.toString());
        });
        test('Should work with standard errors, an error code and params', () => {
            ...
            if ('captureStackTrace' in Error) {
                expect(-1 !== (err.stack || '').indexOf('Error: E_ERROR')).toBeTruthy();
                expect(-1 !==
                    (err.stack || '').indexOf('YError: E_ERROR_2 (["arg1","arg2"])')).toBeTruthy();
            }
            expect(err.name).toEqual(err.toString());
        });
        test('Should work with several wrapped errors', () => {
            ...
            if ('captureStackTrace' in Error) {
                expect(-1 !== (err.stack || '').indexOf('Error: E_ERROR_1')).toBeTruthy();
                expect(-1 !==
                    (err.stack || '').indexOf('YError: E_ERROR_2 (["arg2.1","arg2.2"])')).toBeTruthy();
                expect(-1 !==
                    (err.stack || '').indexOf('YError: E_ERROR_3 (["arg3.1","arg3.2"])')).toBeTruthy();
            }
            expect(err.name).toEqual(err.toString());
        });
    });
    describe('.cast()', () => {
        ...
        test('Should let YError instances pass through', () => {
            ...
            expect(err.debugValues).toEqual(['arg1', 'arg2']);
            if ('captureStackTrace' in Error) {
                expect(-1 !== (err.stack || '').indexOf('YError: E_ERROR (["arg1","arg2"])')).toBeTruthy();
            }
            expect(err.name).toEqual(err.toString());
        });
    });
    describe('.bump()', () => {
        ...
        test('Should work with YError like errors', () => {
            ...
            baseErr.debugValues = ['baseParam1', 'baseParam2'];
            const err = YError.bump(baseErr);
            expect(err.code).toEqual('E_A_NEW_ERROR');
            expect(err.wrappedErrors.length).toEqual(1);
            expect(err.debugValues).toEqual(['baseParam1', 'baseParam2']);
            if ('captureStackTrace' in Error) {
                expect(-1 !== (err.stack || '').indexOf('Error: E_A_NEW_ERROR')).toBeTruthy();
                expect(-1 !==
                    (err.stack || '').indexOf('YError: E_A_NEW_ERROR (["baseParam1","baseParam2"])')).toBeTruthy();
            }
            expect(err.name).toEqual(err.toString());
        });
        test('Should work with Y errors and a message', () => {
            ...
            if ('captureStackTrace' in Error) {
                expect(-1 !==
                    (err.stack || '').indexOf('YError: E_ERROR (["arg1.1","arg1.2"])')).toBeTruthy();
                expect(-1 !==
                    (err.stack || '').indexOf('YError: E_ERROR (["arg1.1","arg1.2"])')).toBeTruthy();
            }
            expect(err.name).toEqual(err.toString());
